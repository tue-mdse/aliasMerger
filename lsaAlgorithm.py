...
#		dataIndices = self.nameEmailData.keys()
		...
		"""Compute Levenshtein similarity between all pairs of terms.
		Only the pairs with at least 50% similarity are considered,
		a priori possible by filtering on term length."""
		start = clock()
		levDocSim = MyDict() # Keeps track of the Levenshtein similarity between terms and documents
		"""Sort the set of terms by length."""
		allTerms = sorted(allTerms, key = lambda term: -len(term))
		end = clock()
		print "Sort terms by length: %6.3f seconds" % (end - start)
		
		start = clock()
		for idx1, term1 in enumerate(allTerms):
#			rowIdx1 = termToRowIndex[term1]
#			levDocSim[rowIdx1] = {} # initialize dictionary
#			for columnIdx in documents.keys():
#				for term2 in documents[columnIdx]:
#					"""For each candidate term, compute normalised Levenshtein similarity."""
#					sim = 1 - float(Levenshtein.distance(term1, term2))/max(len(term1), len(term2))
#	#				sim = 1 - float(editdist.distance(term1, term2))/max(len(term1), len(term2))
#					
#					"""Only keep the pairs that exceed a given threshold."""
#					if sim >= levThr:
#						"""term2 with which term1 is Levenshtein similar appears in these documents:"""
##						docs = termToColumnIndex[term2]
##						for columnIdx in docs:
#						"""Update the max Levenshtein similarity between term1 and the corresponding docs of term2."""
#						try:
#							"""I have previously seen the pair (term1, doc[columnIdx]). Update max similarity if necessary."""
#							if sim > levDocSim[rowIdx1][columnIdx]:
#								levDocSim[rowIdx1][columnIdx] = sim
#						except:
#							"""It's the first time I see (term1, doc[columnIdx]), so max similarity = sim"""
#							levDocSim[rowIdx1][columnIdx] = sim
			
			"""For each term compute a set of candidate terms with which to do Levenshtein comparisons.
			Given term t, only those terms at least half, or at most twice the length of t are interesting.
			=> Normalised Levenshtein similarity at least half."""
			candidates = [term for term in allTerms[idx1+1:] if 2*len(term) >= len(term1)]
			candidates += [term for term in allTerms[:idx1] if 2*len(term1) >= len(term)]
			rowIdx1 = termToRowIndex[term1]
			levDocSim[rowIdx1] = {} # initialize dictionary
			for term2 in candidates:
				"""For each candidate term, compute normalised Levenshtein similarity."""
				sim = 1 - float(Levenshtein.distance(term1, term2))/max(len(term1), len(term2)) # it turns out this is the fastest
#				sim = 1 - float(editdist.distance(term1, term2))/max(len(term1), len(term2))
#				sim = 1 - do_stringcmp('editdist', term1, term2)[0]/max(len(term1), len(term2))
		...
		print "Levenshtein similarity: %6.3f seconds" % (end - start)
		levDocSim.save(os.path.relpath("../data/levenshtein.dict"))
		
		"""Adjust all weights using inverse document frequency to down-vote common terms (e.g., john).
		Weigh each term by log(numDocuments/denominator).
		Part 1 - compute weights"""
		start = clock()
		idf = MyDict()
		for term in allTerms:
			rowIdx = termToRowIndex[term]
			appearsIn = termToColumnIndex[term]
			"""Compute the denominator as the sum of weights."""
			denominator = 0
			for columnIdx in appearsIn:
				"""The weights are 1 for the documents in which the term appears."""
				denominator += 1
			try:
				"""The weights are the Levenshtein similarities for the documents in which the term does not appear."""
				otherColumns = set(levDocSim[rowIdx].keys())
				for columnIdx in otherColumns.difference(appearsIn):
					denominator += levDocSim[rowIdx][columnIdx]
			except:
				pass
			idf[term] = math.log(float(numDocuments)/float(denominator))
		end = clock()
		print "Inverse document frequency: %6.3f seconds" % (end - start)
		
		
		"""Part 2 - Do the actual weighing."""
		start = clock()
		"""Store the doc-term matrix in this sparse format: lists of row/column indices, and list of values."""
		row = []
		col = []
		data = []
		for term in allTerms:
			rowIdx = termToRowIndex[term]
			"""These are the 1s (terms that appear in documents)."""
			for columnIdx in termToColumnIndex[term]:
				row.append(rowIdx)
				col.append(columnIdx)
				data.append(idf[term])
			"""These are the terms that are Levenshtein similar to documents."""
			try:
				extraColumns = set(levDocSim[rowIdx].keys()).difference(termToColumnIndex[term])
				for columnIdx in extraColumns:
					val = idf[term] * levDocSim[rowIdx][columnIdx]
					row.append(rowIdx)
					col.append(columnIdx)
					data.append(val)
			except:
				pass
		"""Create the doc-term matrix (first: to coordinate sparse; second: to csc). scipy does this."""
		A = coo_matrix((data,(row,col)))#, shape=(numTerms,numDocuments))
		A = A.tocsc()
		

		"""Transform to corpus format (gensim)"""
		corpus = matutils.Sparse2Corpus(A)
		corpora.MmCorpus.serialize(os.path.relpath("../data/corpus.mm"), corpus)
		end = clock()
		print "Built corpus: %6.3f seconds" % (end - start)
		
		
		"""Dimensionality reduction: k ratio of the number of terms."""
		maxk = int(round(k * numTerms))
		
		
		"""Compute LSI model (gensim)."""
		start = clock()
		lsi = models.LsiModel(corpus, num_topics=maxk)
		end = clock()
		print "LSI model: %6.3f seconds" % (end - start)
		
		
		"""Compute cosine similarities between each pair of documents (gensim)."""
		start = clock()
...
#		writer.writerow(['left','right','textLeft','textRight','sim','kind'])
			...
			for docIdx2, val in enumerate(simi):
				if docIdx1 < docIdx2:
					if val >= simThr:
						...
						for id1 in aliases1:
							for id2 in aliases2:
								writer.writerow(list(self.nameEmailData[id1])+list(self.nameEmailData[id2])+[str(val)])
		for docIdx in documents.keys():
			aliases = reverseLookup[docIdx]
			for id1, id2 in itertools.combinations(aliases, 2):
				writer.writerow(list(self.nameEmailData[id1])+list(self.nameEmailData[id2])+[str(val)])
